...
// File: contracts/interfaces/IClimbLottery.sol

pragma solidity ^0.8.4;

interface IClimbLottery {
    /**
    ...
    /**
     * @notice Draw the final number, calculate reward in MARS per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in MARS token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInMars: price of a ticket in MARS
 ...
 */
contract ClimbLottery is ReentrancyGuard, IClimbLottery, Ownable {
    ...
    uint256 public maxPriceTicketInMars = 50 ether;
    uint256 public minPriceTicketInMars = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256[6] marsPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInMars;
        uint32 finalNumber;
    }
     ...
     * @param _marsTokenAddress: address of the MARS token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _marsTokenAddress, address _randomGeneratorAddress) {
        ...
        nonReentrant
    {
        ...
        // Calculate number of MARS to this contract
        uint256 amountMarsToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInMars,
            _ticketNumbers.length
        );

        // Transfer mars tokens to this contract
        marsToken.safeTransferFrom(address(msg.sender), address(this), amountMarsToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInMars += amountMarsToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInMarsToTransfer
        uint256 rewardInMarsToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInMarsToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        marsToken.safeTransfer(msg.sender, rewardInMarsToTransfer);

        emit TicketsClaim(msg.sender, rewardInMarsToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInMars) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in MARS for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].marsPerBracket[j] =
                ...
                // A. No MARS to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].marsPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInMars - amountToShareToWinners);

        // Transfer MARS to treasury address
        marsToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        _lotteries[_lotteryId].amountCollectedInMars += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInMars >= minPriceTicketInMars) && (_priceTicketInMars <= maxPriceTicketInMars),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInMars: _priceTicketInMars,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            marsPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInMars: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(marsToken), "Cannot be MARS token");
    ...
    /**
     * @notice Set MARS price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInMars: minimum price of a ticket in MARS
     * @param _maxPriceTicketInMars: maximum price of a ticket in MARS
     */
    function setMinAndMaxTicketPriceInMars(uint256 _minPriceTicketInMars, uint256 _maxPriceTicketInMars)
        external
        onlyOwner
    {
        require(_minPriceTicketInMars <= _maxPriceTicketInMars, "minPrice must be < maxPrice");

        minPriceTicketInMars = _minPriceTicketInMars;
        maxPriceTicketInMars = _maxPriceTicketInMars;
    }
     ...
     * @param _priceTicket price of a ticket (in MARS)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].marsPerBracket[_bracket];
        } else {
            return 0;
        }
    }
